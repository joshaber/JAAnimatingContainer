...
@implementation NSResponder (DemoViewExtensions)

- (void)demoViewWasDeleted:(DemoView *)demoView {
    [[self nextResponder] demoViewWasDeleted:demoView];
}
...
- (IBAction)deleteView:(id)sender {
    [[self nextResponder] demoViewWasDeleted:self];
}
...
- (NSGradient *)gradient {
    if(gradient == nil) {
        self.gradient = [[NSGradient alloc] initWithStartingColor:[NSColor colorWithDeviceWhite:0.8f alpha:1.0f] endingColor:[NSColor colorWithDeviceWhite:0.85f alpha:1.0f]];
    }
